...
// We just need the calculator stuff. Easiest way:
// Replace `activeView === 'calculator' && (` with just `true && (` or `return (` 
// Instead of messing with AST, I will just convert the PortalHubPage back into CalculatorPage and remove hub/quote/service/login blocks.

// Rename component
content = content.replace('export default function PortalPage() {', 'export default function CalculatorPage() {\n  const activeView = "calculator";\n');

// Drop hub render blocks
content = content.replace(/activeView === 'hub' && \([\s\S]*?\n      \)\}/g, '');
content = content.replace(/activeView === 'quote' && \([\s\S]*?\n          \)\}/g, '');
content = content.replace(/activeView === 'service' && \([\s\S]*?\n          \)\}/g, '');
content = content.replace(/activeView === 'login' && \([\s\S]*?\n          \)\}/g, '');

// Clean remaining activeView wrappers
content = content.replace(/activeView !== 'hub' && \(/g, '(');

content = content.replace(/<button className="minimal-back-btn" onClick=\{\(\) => setActiveView\('hub'\)\} style=\{\{ marginBottom: '3rem' \}\}>/g, '<Link href="/portal" className="minimal-back-btn" style={{ marginBottom: "3rem", display: "inline-flex", alignItems: "center", gap: "0.5rem", color: "#666", textDecoration: "none" }}>');
